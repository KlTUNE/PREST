...
#読み取るプログラム
tmp_key, none, tmp_cam_num, tmp_cam_name = prest.setting_read()
move_key = tmp_key[0]
back_key = tmp_key[1]
...
def change_scene(self, backornext):
    global round, checkboclayout, finger, self2, move_key, back_key, finish_key, roop, camera_number, show_performance, default_hands, setting_file_exist, tmp_cam_num, cam_show, reshow, all_keys
    ...
    if scene == 11:
        if save_file:
            self.ids.main_label.text = "設定を保存しました、ボタンを終了します"
        else:
